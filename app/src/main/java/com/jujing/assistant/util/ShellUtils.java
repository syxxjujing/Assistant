...
//            Toast.makeText(MainActivity.this, "跳转失败！！！！", 1).show();
            t.printStackTrace();
        }
    }

    public static boolean forceAdb(String adb) {
        String[] shell = new String[]{adb};

        int result = ShellUtils.execCommand(shell, true).result;
        Log.e("log_", "result297: " + result+"--->"+adb);
        if (result != 0) {
            forceAdb(adb);
        } else {
    ...
    public static boolean forceAdbArray(String[] shell){
        int result = ShellUtils.execCommand(shell, true).result;
        Log.e("log_", "result316: " + result);
        if (result != 0) {
            forceAdbArray(shell);
        } else {
    ...
    public static void forceAdb2(String adb) {
